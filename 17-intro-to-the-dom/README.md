...
```
  ...
  - get the `span` from inside the `li` element
...
1. grab the img tag from the DOM somehow
2. update its src property to show the new picture url
...
2. update _______ to show the new text

```javascript
let title = document.querySelector("h1")
title.textContent = "Baby Sloths Are Cool! 🙊"
```
---
...
```javascript
// let jaws = document.querySelect('li')
li.remove()
```
